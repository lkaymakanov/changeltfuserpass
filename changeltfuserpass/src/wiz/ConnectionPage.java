...
import user.User;
import user.UserSelectStatement;
...
public class ConnectionPage extends Page {
	...
	CONTYPE conType = CONTYPE.XMLFILE;   //xml file default
	...
	public ConnectionPage(Wizard w) {
    ...
    public void Connect(){
    	//get data source & execute query
    	DataSourceConnectionFactoryDrManager ds = new  DataSourceConnectionFactoryDrManager(getConnectionProperties());
    	DBExecutor ex = new  DBExecutor(ds);
    	
    	UserSelectStatement st = new UserSelectStatement();
		ex.execute(st);
		
		List<User> usrs = st.getResult();
		
		//init edit user & text fields
		w.getUserPage().initTextFields();
		
		//set data in grid
		w.userPage.getUsertbl().setUsers(usrs);
    	/*DataSourceFactoryBypass ds = new DataSourceFactoryBypass(getConnectionProperties());
       	Connection c = ds.getConnection();
    	
       	if(ds.getErr().getErr() == ErrCodeMsg.ERRCODE.ERR){return ds.getErr();}
    	
    	DBExecutor ex = new DBExecutor(c);
		UserSelectStatement st = new UserSelectStatement();
		ex.execute(st);
		
		if(ds.getErr().getErr() == ErrCodeMsg.ERRCODE.ERR) {return ex.getErr();}
		List<AbstractModel> usrs = st.getResult();
  ...
  //get connection from file
    private ConnectionProperties getPropertiesFromFile(){
    	...
    	if(connectionsCombo.getSelindex() != -1) return connectionsCombo.prop.get(connectionsCombo.getSelindex()); 
